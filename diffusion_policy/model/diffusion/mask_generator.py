...
class LowdimMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        ...
        rng = torch.Generator(device=gen_device)
        if seed is not None:
        ...
        is_obs_dim = torch.logical_not(is_action_dim)
        ...
        if self.fix_obs_steps:
            ...
            obs_steps = torch.randint(
                ...
                size=(B,), generator=rng, device="cpu").to(device)
...
class KeypointMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        ...
        if self.time_independent:
            visible_kps = (torch.rand((B, T, n_keypoints), generator=rng, device="cpu")
               < self.keypoint_visible_rate).to(device)
            ...
            visible_kps = (torch.rand((B, n_keypoints), generator=rng, device="cpu")
               < self.keypoint_visible_rate).to(device)
            visible_dims = torch.repeat_interleave(visible_kps, repeats=self.keypoint_dim, dim=-1)
            visible_dims_mask = torch.cat([
